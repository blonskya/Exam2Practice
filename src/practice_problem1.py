...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(contents) <= volume:
            ...
            self.contents=''
        self.origvol=self.volume
        self.origcont=self.contents
        self.hist = []
    ...
    def append_string(self, additional_contents):
        ...
        if self.volume-len(self.contents) < len(additional_contents):
            sigma=(self.volume-len(self.contents))
            for k in range (sigma):
                self.contents += additional_contents[k]
            for k in range (sigma,len(additional_contents)):
    ...
    def shrink(self, new_volume):
        ...
        if new_volume < len(self.contents):
            for k in range(new_volume):
                newcont += self.contents[k]
            for k in range(new_volume,len(self.contents)):
                oldcont += self.contents[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        ex = len(self.double())
        why = len(self.shrink(new_volume))
        return ex+why
    ...
    def reset(self):
        ...
        for k in range (len(self.contents)):
            etherium += self.contents[k]
        self.hist += [etherium]
        self.volume = self.origvol
        self.contents = self.origcont
    ...
    def steal(self, other_box):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        drppd = self.append_string(other_box.contents)
        other_box.contents=drppd
    ...
    def combined_box(self, other_box):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        conts = self.contents + other_box.contents
        vole = self.volume + other_box.volume
        return(Box(conts,vole))
